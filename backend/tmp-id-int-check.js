...
(async () => {
  ...
  const [rows] = await c.query("SELECT TABLE_NAME,COLUMN_NAME,COLUMN_TYPE,COLUMN_KEY,EXTRA FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND COLUMN_NAME='id_int' ORDER BY TABLE_NAME");
  console.table(rows);
  await c.end();
