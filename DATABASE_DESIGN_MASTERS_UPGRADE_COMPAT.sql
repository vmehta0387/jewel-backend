-- Design Masters Upgrade (Compatibility Version)
-- Same intent as DATABASE_DESIGN_MASTERS_UPGRADE.sql, but avoids
-- "ADD COLUMN IF NOT EXISTS" syntax for broader MySQL compatibility.
...
SET @col_sql := IF(
  @col_exists = 0,
  'ALTER TABLE design_masters ADD COLUMN normalized_alias VARCHAR(255) NULL AFTER alias_name',
  'SELECT 1'
);
PREPARE stmt FROM @col_sql;
EXECUTE stmt;
...
SET @col_sql := IF(
  @col_exists = 0,
  'ALTER TABLE design_masters ADD COLUMN description TEXT NULL AFTER normalized_alias',
  'SELECT 1'
);
PREPARE stmt FROM @col_sql;
EXECUTE stmt;
...
SET @col_sql := IF(
  @col_exists = 0,
  'ALTER TABLE design_masters ADD COLUMN finding_no VARCHAR(100) NULL AFTER description',
  'SELECT 1'
);
PREPARE stmt FROM @col_sql;
EXECUTE stmt;
...
SET @col_sql := IF(
  @col_exists = 0,
  'ALTER TABLE design_masters ADD COLUMN metal_caratage VARCHAR(100) NULL AFTER finding_no',
  'SELECT 1'
);
PREPARE stmt FROM @col_sql;
EXECUTE stmt;
...
SET @col_sql := IF(
  @col_exists = 0,
  'ALTER TABLE design_masters ADD COLUMN price_in ENUM(''PIECES'', ''GRAM'', ''PAIR'', ''INCHES'') NULL AFTER metal_caratage',
  'SELECT 1'
);
PREPARE stmt FROM @col_sql;
EXECUTE stmt;
...
SET @col_sql := IF(
  @col_exists = 0,
  'ALTER TABLE design_masters ADD COLUMN price_per_unit DECIMAL(12,2) NULL AFTER price_in',
  'SELECT 1'
);
PREPARE stmt FROM @col_sql;
EXECUTE stmt;
...
SET @col_sql := IF(
  @col_exists = 0,
  'ALTER TABLE design_masters ADD COLUMN dimensions VARCHAR(255) NULL AFTER price_per_unit',
  'SELECT 1'
);
PREPARE stmt FROM @col_sql;
EXECUTE stmt;
...
SET @col_sql := IF(
  @col_exists = 0,
  'ALTER TABLE design_masters ADD COLUMN weight_per_unit DECIMAL(12,3) NULL AFTER dimensions',
  'SELECT 1'
);
PREPARE stmt FROM @col_sql;
EXECUTE stmt;
...
SET @designs_col_exists := (
  SELECT COUNT(1)
  FROM information_schema.columns
  WHERE table_schema = DATABASE()
    AND table_name = 'designs'
    AND column_name = 'diamond_spread'
);
SET @designs_col_sql := IF(
  @designs_col_exists = 0,
  'ALTER TABLE designs ADD COLUMN diamond_spread VARCHAR(100) NULL AFTER stage',
  'SELECT 1'
);
PREPARE stmt3 FROM @designs_col_sql;
EXECUTE stmt3;
...
SET @designs_col_exists := (
  ...
  WHERE table_schema = DATABASE()
    AND table_name = 'designs'
    AND column_name = 'diamond_type'
);
SET @designs_col_sql := IF(
  @designs_col_exists = 0,
  'ALTER TABLE designs ADD COLUMN diamond_type VARCHAR(120) NULL AFTER diamond_spread',
  'SELECT 1'
);
PREPARE stmt3 FROM @designs_col_sql;
EXECUTE stmt3;
